...
curl -k --cert-type P12 --cert keystores/${client_name}-keystore.p12:$client_password https://localhost:8081/actuator/health
curl -k --cert-type P12 --cert keystores/${client_name}-keystore.p12:$client_password https://localhost:8082/actuator/health
...
iptables -A INPUT -p tcp --dport 8081 -s <gateway-ip> -j ACCEPT
iptables -A INPUT -p tcp --dport 8081 -j DROP
iptables -A INPUT -p tcp --dport 8082 -s <gateway-ip> -j ACCEPT
iptables -A INPUT -p tcp --dport 8082 -j DROP

# Allow infrastructure services from internal network only
iptables -A INPUT -p tcp --dport 8888 -s <internal-network> -j ACCEPT
